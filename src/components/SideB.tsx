...
export default function SideB({ onBack }: { onBack: () => void }) {
    ...
    const [isHostGame, setIsHostGame] = useState(() => {
        try { return localStorage.getItem('sideb_is_host_game') === 'true'; } catch { return false; }
    });
    const [bgioPlayerID, setBgioPlayerID] = useState<string>(() => {
        try {
            let val = localStorage.getItem('sideb_bgio_player_id');
            if (!val) {
                val = localStorage.getItem('oracle_user_id') || Math.random().toString(36).substring(7);
                localStorage.setItem('sideb_bgio_player_id', val);
            }
            return val;
        } catch {
    ...
    useEffect(() => {
        try {
            localStorage.setItem('sideb_is_host_game', String(isHostGame));
        } catch {}
    }, [isHostGame]);
    ...
    const createGame = (type: 'UNO' | 'TEBAKKATA' = 'TEBAKKATA') => {
        ...
        const pid = localStorage.getItem('sideb_bgio_player_id') || localStorage.getItem('oracle_user_id') || Math.random().toString(36).substring(7);
        localStorage.setItem('sideb_bgio_player_id', pid);
        setBgioPlayerID(pid);
        ...
        >
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            {(() => {
                                const activeBPlayers = serverActivePlayers.filter(u => u.room === 'B');
                                const displayCount = activeBPlayers.length > 0 ? activeBPlayers.length : onlineUsers.filter(u => u.room === 'B').length;
                                if (displayCount === 0) return null;
                                return (
                                    <div className="group relative flex items-center gap-1.5 bg-purple-500/10 px-2 py-0.5 rounded-full border border-purple-500/20 text-[10px] text-purple-300 font-mono cursor-pointer transition-all duration-300 hover:bg-purple-500/25">
                                        <span className="w-1.5 h-1.5 rounded-full bg-purple-400 animate-pulse" />
                                        <span>{displayCount} SISI B ACTIVE</span>
                                        
                                        {/* Pure-CSS hover tooltip dropdown */}
                                        <div className="absolute right-0 top-full mt-2 w-48 bg-zinc-950/95 border border-purple-500/30 rounded-xl p-2.5 shadow-2xl z-50 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                            <p className="text-[9px] text-zinc-400 font-bold uppercase tracking-wider mb-1.5 border-b border-purple-500/20 pb-1">Live Players in Room B:</p>
                                            <div className="space-y-1 max-h-32 overflow-y-auto custom-scrollbar">
                                                {activeBPlayers.length > 0 ? (
                                                    activeBPlayers.map((u, idx) => (
                                                    ...
                                                    onlineUsers.filter(u => u.room === 'B').map((u, idx) => (
                                                        <div key={idx} className="flex items-center gap-1.5 text-white/90 text-[10px]">
                                                            <span className="w-1.5 h-1.5 rounded-full bg-purple-400" />
                                                            <span className="truncate text-purple-300">{u.username || 'Anonymous'}</span>
                                                        </div>
                                                    ))
                                                )}
                                            </div>
            ...
            {showUnoBoard && (
                <SupabaseMultiplayerWrapper 
                    ...
                    playerName={username}
                    isHost={isHostGame}
                    onLeave={(isHostPlayer) => {
                        if (actualMatchId && isHostPlayer) {
