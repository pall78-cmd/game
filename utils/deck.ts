...
export type CardType = 'truths' | 'dares' | 'wildcards';
...
const CHAOS_WILDCARDS = [
    "VN desah or foto bibir.", "Cerita mimpi basah or pap paha.", "Pap outfit tidur or jujur lampu.", "Dominant or Submissive?", "Satu kata soal nafsu.", "Pilih member desah VN.", "Cerita ciuman pertama.", "Spit or swallow?", "Pilih orang kirim foto leher.", "Turn on terbesar?", "Awkward saat intim?", "Pagi or malam buat 'itu'?", "Member VN suara berat.", "Pernah FWB?", "SS galeri tersembunyi.", "Suka oral?", "Sebut ukuran ideal.", "Pernah sexting?", "Lokasi main impian.", "Suka lingerie warna apa?", "Pilih member kirim foto tangan.", "Pernah one night stand?"
];
...
export const drawCardSpecific = (currentDeck: GameDeckState, intensity: Intensity, type: CardType): { content: string, newDeck: GameDeckState } => {
    ...
    saveDeck(newDeck);
    return { content, newDeck };
};

// Fix for missing drawCard export used in App.tsx
// This helper picks a random category (truth/dare/wildcard) for a given intensity.
export const drawCard = (currentDeck: GameDeckState, intensity: Intensity): { content: string, newDeck: GameDeckState } => {
    const types: CardType[] = ['truths', 'dares', 'wildcards'];
    const type = types[Math.floor(Math.random() * types.length)];
    return drawCardSpecific(currentDeck, intensity, type);
};
